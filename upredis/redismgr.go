...
func init(){
	db, err := upgin.AppConfig.Int("redis.db")
	if err != nil {
		db = 0
	}
...
func RedisInit(db int, redism *RedisMgr){
	...
	//最大连接数
	maxActive, err := upgin.AppConfig.Int("redis.maxActive")
	if err != nil{
		maxActive = 600
	}
	//空闲链接超时时间
	idleTimeout, err := upgin.AppConfig.Int64("redis.idleTimeout")
	if err != nil{
		idleTimeout = 600
	}
	//如果超过最大连接，是报错，还是等待
	wait, err := upgin.AppConfig.Bool("redis.wait")
	if err != nil{
		wait = true
	}
	timeout := time.Duration( idleTimeout )  * time.Second

	pass := upgin.AppConfig.String("redis.pass")
