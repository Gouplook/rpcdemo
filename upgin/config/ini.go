...
func (ini *IniConfig)Parse(name string)(Configer, error) {
	return ini.parseFile(name, defaultSection)
}


func (ini *IniConfig) parseFile(name string, section string) (*IniConfigContainer, error) {
	data, err := ioutil.ReadFile(name)
	if err != nil {
		return nil, err
	}

	return ini.parseData(filepath.Dir(name), data, section)
}

func (ini *IniConfig) parseUrl(url string, section string) (*IniConfigContainer, error) {
	...
	return ini.parseData("", data, section)
}

func (ini *IniConfig) parseData(dir string, data []byte, section string) (*IniConfigContainer, error) {
	...
	for {
		...
		// Identify end-of-line comments
		var bSuffixComment []byte
		var suffix []byte
		if index := bytes.Index(line, bNumComment); index != -1 {
			bSuffixComment = bNumComment
			suffix = line[index:]
			line = line[:index]
		}else if index = bytes.Index(line, bSemComment) ; index != -1 {
			bSuffixComment = bSemComment
			suffix = line[index:]
			line = line[:index]
		}
		if bSuffixComment != nil {
			suffix = bytes.TrimLeft(suffix, string(bSuffixComment))
			// Need append to a new line if multi-line comments.
			if comment.Len() > 0 {
				comment.WriteByte('\n')
			}
			comment.Write(suffix)
		}
		...
		if len(keyValue) == 1 && strings.HasPrefix(key, "include") {
			...
			if (includefiles[0] == "include" || includefiles[0] == "include_url" ) && len(includefiles) == 2 {

				i := &IniConfigContainer{}
				if includefiles[0] == "include" {
					otherfile := strings.Trim(includefiles[1], "\"")
					if !filepath.IsAbs(otherfile) {
						otherfile = filepath.Join(dir, otherfile)
					}

					i, err = ini.parseFile(otherfile, section)
					if err != nil {
						// return nil, err
						continue
					}
				}else if includefiles[0] == "include_url" {
					otherfile := strings.Trim(includefiles[1], "\"")
					i, err = ini.parseUrl(otherfile, section)
					if err != nil {
						// return nil, err
						continue
					}
...
func (ini *IniConfig) ParseData(data []byte) (Configer, error) {
	...
	return ini.parseData(dir, data, defaultSection)
}
