...
var (
	...
	beeTemplateExt = []string{"tpl", "html", "gohtml", "tmpl"}
...
func getTplDeep(root string, fs http.FileSystem, file string, parent string, t *template.Template) (*template.Template, [][]string, error) {
	...
	reg := regexp.MustCompile(UpConfig.WebConfig.TemplateLeft + "[ ]*template[ ]+\"([^\"]+)\"")
...
func getTemplate(root string, fs http.FileSystem, file string, others ...string) (t *template.Template, err error) {
	t = template.New(file).Delims(UpConfig.WebConfig.TemplateLeft, UpConfig.WebConfig.TemplateRight).Funcs(beegoTplFuncMap)
	var subMods [][]string
	...
	for _, m := range subMods {
		if len(m) == 2 {
			...
			for _, otherFile := range others {
				...
				reg := regexp.MustCompile(UpConfig.WebConfig.TemplateLeft + "[ ]*define[ ]+\"([^\"]+)\"")
				allSub := reg.FindAllStringSubmatch(string(data), -1)
